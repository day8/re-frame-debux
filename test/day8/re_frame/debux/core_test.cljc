...
(defn macroexpand-mine
  [quoted]
  (let [form quoted
        env {}]
    (if (seq? form)
      (loop [form form form' (cljs.analyzer/macroexpand-1 env form)]
        (if-not (identical? form form')
          (recur form' (cljs.analyzer/macroexpand-1 env form'))
          `(quote ~form')))
      form)))
...
(deftest simple-dbgn-test
  (let [f '(inc 1)
    ...
    (println "F3" f3)
    (println "F4" f4)
    (println "EXPAND F4" (macroexpand-mine f4))
    (is (= (eval f4) 2))
    (is (= @traces
