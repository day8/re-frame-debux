...
`set-tap-output!`, `set-trace-frames-output!`, `set-date-time-fn!`,
`set-print-length!`, `set-print-seq-length!`, `reset-indent-level!` —
ship as no-op `defn`s so app-boot wiring keeps compiling.
