(ns day8.re-frame.tracing.runtime
  "Production stub for the runtime API. The dev-side namespace at
   ...
   replacement; in a release build (two-jar setup) this stub takes
   over and compiles every wrap-handler! / wrap-event-fx! /
   wrap-event-ctx! / wrap-sub! / wrap-fx! call down to a bare
   `re-frame.core/reg-event-db` (or sibling) — no `fn-traced` wrap,
   no zipper walking, zero per-handler trace cost.
